...
- **자동화(3B)는 3A를 통과한 뒤에만 실행됩니다.** 일회성 동작(`launch_app`, `run_script`)은 `priority`
  단위로 묶어 실행합니다 — 낮은 것이 먼저, 같은 우선순위는 동시에, 한 묶음이 끝난 뒤 다음 묶음으로
  진행하고, 한 묶음 안의 실패가 뒤따르는 묶음을 막지 않습니다. 일치 실패(Conflict)와 실행 실패(Error)는
  서로 다른 상태이며 별도로 보고됩니다.
...
- 트레이 팝업 패널: 실시간 상태, 모든 활성 인터페이스, VPN 터널, 일치 배지가 있는 원클릭 Profile 전환,
  언어 전환. 왼쪽 클릭으로 열고, 포커스를 잃으면 접히며, 오른쪽 클릭은 네이티브 메뉴를 보여줍니다.
...
NetSense는 트레이에 상주합니다: 왼쪽 클릭으로 패널, 오른쪽 클릭으로 메뉴 (설정 · 로그 폴더 열기 ·
현재 네트워크를 DHCP로 · 지금 점검 · 종료; 그 위에는 읽기 전용 행으로 사용 중인 NIC와 터널이
표시됩니다). 구성은 NetSense 실행 파일과 같은 위치의 단일 `config.json`입니다. `config.example.json`은
완성된 실전 예시입니다.

> Windows에서는 첫 네트워크 변경 때 UAC가 한 번 표시됩니다. 관리자로 한 번 실행하면 사라지고, 이후 권한
> 채널은 "승인 불필요"로 보고됩니다.
