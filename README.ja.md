...
- **適用は 1 つのバリア（3A）です**: IPv4 / サブネットマスク / ゲートウェイ / DNS / IPv6 と静的ルートが
  ...
  復帰します。
- **自動化（3B）は 3A を通過した後だけに実行されます。** 単発アクション（`launch_app`、`run_script`）は
  `priority` ごとにバッチで実行されます —— 低いものが先、同じ優先度は並行、あるバッチが終わってから
  次のバッチへ進み、1 つのバッチ内の失敗が後のバッチを止めることはありません。一致の失敗（Conflict）と
  実行の失敗（Error）は別の状態であり、分けて報告されます。
...
- トレイのポップアップパネル: リアルタイムの状態、有効なすべてのインターフェース、VPN トンネル、
  一致バッジ付きのワンクリック Profile 切り替え、言語切替。左クリックで開き、フォーカスが外れると
  自動で収まり、右クリックでネイティブメニューが出ます。
...
NetSense はトレイに常駐します: 左クリックでパネル、右クリックでメニュー（設定 · ログフォルダを開く ·
現在のネットワークを DHCP にする · いま確認 · 終了。その上には読み取り専用の行として、使用中の NIC と
トンネルが表示されます）。設定は NetSense の実行ファイルと同じ場所にある 1 つの `config.json` です。
