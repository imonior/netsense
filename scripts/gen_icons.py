...
Output (src-tauri/icons/)
    ...
    icon.ico    16/24/32/48/64/128/256 — all stored as uncompressed BMP frames.
               (A PNG-compressed frame makes the NSIS bundled with Tauri silently fall
                back to its default installer icon, so we keep every ICO frame as BMP.)
...
# Sizes >= ICO_PNG_FROM are stored as PNG-compressed frames. Our ICO tops out at 256,
# so keeping this above 256 means EVERY frame is an uncompressed BMP DIB. We deliberately
# avoid PNG frames: the NSIS tool bundled with Tauri cannot embed a PNG-compressed .ico
# and silently falls back to its default installer icon (the bug behind the Windows
# "installer icon is still NSIS's" report). BMP frames are rendered correctly by Explorer
# and every NSIS version.
ICO_PNG_FROM = 512
ICNS_CHUNKS = (
    (b"ic07", 128),
    (b"ic08", 256),
